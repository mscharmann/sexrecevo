...
def get_commandline_arguments ():
	...
	parser.add_argument("-e", required=True, help="effect size of reduction in recombination probability, between 0 and 1", metavar="INT")
...
def make_dioecious_pop (N_per_cell, grid_xlen, grid_ylen, sexdet_site):
	...
	for x in range(grid_xlen):
		...
		for y in range(grid_ylen):
			pop[x].append([])
			pop[x][y] = []
			for i in range(int(0.5*N_per_cell)):
				cnt += 1
...
def fake_meiosis (pop, variant_idxes, variant_effects, variant_areas, grid_xlen, grid_ylen, n_gametes_per_individual, chrom_length, nominal_rec_rate_array, record_rec_probs):
...
def mutate_gametes(ingametes, grid_xlen, grid_ylen, mu, chrom_length, variant_idxes, variant_effects, variant_areas, new_area_size, new_effect):
	...
	#	heteroz_effect_sizes = -1*numpy.random.beta(1, 0.01, size = total_expected_mutations)
		heteroz_effect_sizes = numpy.array([-new_effect]*total_expected_mutations)
...
logf = int(args.logf)
logw = int(args.logw)
new_area_size = int(args.a) 
new_effect = float(args.e)
...
#	print "cleaning"
	if (float(generation)/20).is_integer(): # every 20 generations clean up the list of mutations
...
#	print "doing meiosis"
	if (float(generation)/logf).is_integer(): # every x generations report something
		...
		report_window_male_female_Fst_and_dxy (pop, variant_idxes, gridlength_x, gridlength_y, 20.0, generation, logw, int(0.5*logw), chrom_length)
		nonrec_size = report_size_of_nonrec_region (generation, avg_X_Y_rec_probs, sexdet_site)
		# we stop the simulation if the nonrec_size reaches the full length of the chromosome, because there is no way back anyway:
		if nonrec_size == chrom_length:
...
#	print "mutating"
	dispersed_pop_gametes, variant_idxes, variant_effects, variant_areas = mutate_gametes(pop_gametes, gridlength_x, gridlength_y, mu , chrom_length, variant_idxes, variant_effects, variant_areas, new_area_size, new_effect)
#	print "mating"
