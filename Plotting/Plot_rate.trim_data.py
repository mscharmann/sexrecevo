...
def Plot_rate(data_directory, generations_lower_cutoff, nonrec_size_upper_cutoff):
    ...
    for dat_file_name in data_files:
        if "logfile_nonrec_region" in dat_file_name:
            ...
            for line in dat_file:
                ...
                if gen >= generations_lower_cutoff:
                    if size < nonrec_size_upper_cutoff:
    ...
    outplot_path = "%s/%s" % (data_directory, "Non_recomb_expansion_rates.trimmed.pdf")
...
if len(sys.argv) < 3:
    ...
    generations_lower_cutoff = int(sys.argv[2])
    nonrec_size_upper_cutoff = int(sys.argv[3])

    Plot_rate(data_directory_cline, generations_lower_cutoff, nonrec_size_upper_cutoff)
