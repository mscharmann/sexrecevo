...
def make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, record_rec_probs):
	...
	for idx,father in enumerate(sampled_parents[2]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (father_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, record_rec_probs)
			male_gametes += [g1,g2]
	...
	for idx,mother in enumerate(sampled_parents[3]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (mother_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, record_rec_probs)
...
def simple_meiosis (parent_genotype, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, record_rec_probs):
	...
	window_size = 22 # an even integer
	...
	# set identity at sexdet locus to 1, even if heterozyogous (XY male meiosis)
	idents[variant_idxes.index(chrom_length/2)] = 1
#	print idents[variant_idxes.index(chrom_length/2)]
	...
	windows_end_idx = chrom_length-(window_size-window_start_idx)
...
for generation in range(int(args.g)+1):
	...
	if (float(generation)/logf).is_integer(): # every x generations report something
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, True)
		...
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, False)
