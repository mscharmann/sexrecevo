...
#					print "is female"
				...
				for meiosis in range(n_meioses): 
					...
					if not does_recombine: # gametes have same haplotypes as parent
						...
						pairwise_identity_per_site_window_avg = []
						while right_idx < len( i[1] ):
							window_idenity_avg = (sum(pairwise_identity_per_site[left_idx:right_idx])/wlenfloat)**10
							print window_idenity_avg
							pairwise_identity_per_site_window_avg.append(window_idenity_avg)
							left_idx += 1
							right_idx += 1
						# add the ends:
...
nominal_overall_rec_rate_per_site = 2e-10 # can be at most 1 event; => max(nominal_overall_rec_rate_per_site) = 1/sequence_length
