...
"""
NO MUTATIONS IN FIRST 100 bases!
...
def make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, record_rec_probs):
	...
	for idx,father in enumerate(sampled_parents[2]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (father_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, record_rec_probs)
			male_gametes += [g1,g2]
	...
	for idx,mother in enumerate(sampled_parents[3]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (mother_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, record_rec_probs)
	...
	if not set(m_recws) == set([""]):
		a = [str(x) for x in m_recws if x != ""]
		with open("logfile_m_recws.txt", "a") as F:
			F.write("\n".join(a)+"\n")
	if not set(f_recws) == set([""]):
		a = [str(x) for x in f_recws if x != ""]
		with open("logfile_f_recws.txt", "a") as F:
			F.write("\n".join(a)+"\n")		
	else:
		a = None
	
	# pseudo-random shuffle the gamete pools, using the generation number. 
	# If gamete pools not randomised, divergence along chromosome can become skewed!
	mrandindex = int(len(male_gametes)*(float(generation)/len(male_gametes) % 1 ))
	frandindex = int(len(female_gametes)*(float(generation)/len(female_gametes) % 1 ))
#	print "before", male_gametes[:3]
	male_gametes = male_gametes[mrandindex:] + male_gametes[:mrandindex]
	female_gametes = female_gametes[frandindex:] + female_gametes[:frandindex]
#	print "after", mrandindex, len(male_gametes), male_gametes[:3]
	
	return male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, a
	
def simple_meiosis (parent_genotype, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, record_rec_probs):
...
#			gamete_2 = parent_genotype[1][:rec_idx_among_variants] + parent_genotype[0][rec_idx_among_variants:]
		...
		if record_rec_probs:
			return gamete_1, gamete_2, per_base_rec_probs_arr, rec_window
		else:
			return gamete_1, gamete_2, "", rec_window
...
for generation in range(int(args.g)+1):
	...
	if (float(generation)/logf).is_integer(): # every x generations report something
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, True)
		...
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, False)
...
#	pop, variant_idxes, sexdet_site = invert_sequence (pop, variant_idxes, sexdet_site, chrom_length)
	if rec_windows:
		all_rec_windows += rec_windows
print "Done!"
with open("fuf.txt", "w") as F:
	F.write("\n".join([str(x) for x in all_rec_windows]) + "\n") 
