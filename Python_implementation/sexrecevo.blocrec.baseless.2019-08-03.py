...
def make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, record_rec_probs):
	...
	for idx,father in enumerate(sampled_parents[2]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs = simple_meiosis (father_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, record_rec_probs)
			male_gametes += [g1,g2]
	...
	for idx,mother in enumerate(sampled_parents[3]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs = simple_meiosis (mother_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, record_rec_probs)
...
for generation in range(int(args.g)+1):
	...
	if (float(generation)/logf).is_integer(): # every x generations report something
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, True)
		...
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, rec_window_size, generation, False)
