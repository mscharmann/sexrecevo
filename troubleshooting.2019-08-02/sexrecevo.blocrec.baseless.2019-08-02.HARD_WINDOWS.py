...
def make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, record_rec_probs):
	...
	for idx,father in enumerate(sampled_parents[2]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (father_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, record_rec_probs)
			male_gametes += [g1,g2]
	...
	for idx,mother in enumerate(sampled_parents[3]):
		...
		for meiosis in range(n_meioses):
			g1, g2, rec_probs, recw = simple_meiosis (mother_genotype, variant_idxes, chrom_length,  nominal_overall_rec_rate_per_site, sexdet_site, record_rec_probs)
...
def simple_meiosis (parent_genotype, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, record_rec_probs):
...
#	print idents[variant_idxes.index(chrom_length/2)]
	...
	window_size = 56 # an even integer smaller than 0.5 * chrom_length to get at least two windows
...
#	print n_windows
	...
	windows_borders = [[window_size*n+window_start_idx,window_size*(n+1)+window_start_idx] for n in range(0,n_windows)]
		...
		if idents[idx] == 0:			
			...
			if variant > window_start_idx:
				if variant < windows_end_idx:
					for windex, window in enumerate(windows_borders):
						if variant >= window[0]:
							if variant < window[1]:
								# print "hit", window, variant
								variant_window_idx = windex
...
for generation in range(int(args.g)+1):
	...
	if (float(generation)/logf).is_integer(): # every x generations report something
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, True)
		...
		male_gametes, female_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs, rec_windows = make_gametes_meioses (pop, N, sampled_parents, variant_idxes, chrom_length, nominal_overall_rec_rate_per_site, sexdet_site, False)
