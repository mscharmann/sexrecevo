...
##
def do_meiosis(pop, variant_idxes, variant_effects, grid_xlen, grid_ylen, n_gametes_per_individual, chrom_length, nominal_rec_rate_array, sliding_window_size, record_rec_probs):
	"""
				...
				for meiosis in range(n_meioses): 
					...
					for vidx, variant in enumerate(variant_idxes):
						...
						if genotype == 1: # heterozygous in this meiosis
							...
							if record_rec_probs:
								specific_probs_per_site[variant-half_of_windowsize:variant+half_of_windowsize] += (nominal_probs_per_site[variant]*variant_effects[vidx][genotype])
...
#					print "the effects", effects
					...
					if len(snp_pos) < 2:
						...
						for p in intervals:
							...
							effect2 = effects[cntb+1]
							prob += effect1*half_of_windowsize
							prob += effect2*half_of_windowsize
							if prob < 0.0:
								prob = 0.0
							interval_probabilities.append(prob)
#							print p, prob, effect1, effect2  
							cntb += 1
...
######################
"""
Problem to be solved: at the moment, adjoining mutations have no overlapping effects; this fine because all effects are -100%; but needs to be properly done later!
"""
########
...
sexdet_site = 50000 ## males = AT, females = TT ; other genotypes are neuters = no gametes! 
...
for generation in range(5000+1):
#	measure_chrom_lengths (pop, gridlength_x, gridlength_y)
	pop = enforce_carrying_capacity (pop, gridlength_x, gridlength_y, 50.0) # carrying capacity must be a float!
#	print "cleaning"
	...
	if n_total == 0.0:
		...
		break
#	print "doing meiosis"
	if (float(generation)/100).is_integer(): # every x generations report something
		pop_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs = do_meiosis(pop, variant_idxes, variant_effects, gridlength_x, gridlength_y, 4, chrom_length, nominal_rec_rate_array, sliding_window_size, True)
		...
		pop_gametes, avg_X_Y_rec_probs, avg_X_X_rec_probs = do_meiosis(pop, variant_idxes, variant_effects, gridlength_x, gridlength_y, 4, chrom_length, nominal_rec_rate_array, sliding_window_size, False)
#	print "dispersing"
